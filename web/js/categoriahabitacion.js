function llenarFormulario(fila) {
    ...
    var codhab = $(fila).find(".codhab").text();
    var tiphab = $(fila).find(".tipohab").text();
    var precio = $(fila).find(".precio").text();

    $("#txtCodigo").val(codigo);
    $("#txtCodhabitacion").val(codhab);
    $("#txtTipohabitacion").val(tiphab);
...
$(document).ready(function () {
    ...
    $("#exampleModal").on("hidden.bs.modal", function () {
        $('form')[0].reset();
        $("#txtPreciohabitacion").val('');  // Limpia el campo de password si es un input de texto
    });
...
function generarPDF() {
    ...
    xhr.onload = function () {
        if (xhr.status == 200) {
            ...
            link.download = 'categorias_habitacion.pdf';
    ...
    var url = contextPath + '/PDFs/CategoriaHabitacionPdf.jsp';
...
function generarExcel() {
    ...
    xhr.onload = function () {
        if (xhr.status == 200) {
            ...
            link.download = 'categorias_habitacion.xls';
    ...
    // Construir la URL para la solicitud al servidor
    var url = contextPath + '/Excels/CategoriaHabitacionExcel.jsp';
